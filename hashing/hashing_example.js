...
function pickServerRenderzvous(username, servers) {
    let maxServer = null;
    let maxScore = null;
...
for(const username of usernames) {
    const server1 = pickServerRenderzvous(username, serverSet1);
    const server2 = pickServerRenderzvous(username, serverSet2);
    const serversAreEqual = server1 === server2;
    console.log(`${username}: ${server1} => ${server2} | equal: ${serversAreEqual}`);
}